...
def main():
    global DEBUG
    parser = argparse.ArgumentParser(description="Loop video or audio")
    subparsers = parser.add_subparsers(dest="output_type", help="Output type", required=True)
    loop_audio = subparsers.add_parser("a", help="Loop audio")
    loop_audio.add_argument("video_path", help="Input audio file")
    loop_audio.add_argument("--start-time", type=float, default=0, help="Start time in seconds")
    loop_audio.add_argument("--end-time", type=float, required=True, help="End time in seconds")
    loop_audio.add_argument("--duration", type=float, help="Target duration in seconds")
    loop_audio.add_argument("--output-dir", help="Output directory")
    loop_audio.add_argument("--debug", action="store_true", help="Enable debug output")
    loop_video = subparsers.add_parser("v", help="Loop video")
    loop_video.add_argument("video_path", help="Input video file")
    loop_video.add_argument("--start-time", type=float, default=0, help="Start time in seconds")
    loop_video.add_argument("--end-time", type=float, required=True, help="End time in seconds")
    loop_video.add_argument("--duration", type=float, help="Target duration in seconds")
    loop_video.add_argument("--output-dir", help="Output directory")
    loop_video.add_argument("--debug", action="store_true", help="Enable debug output")
    ...
    prefix = "AL" if output_type == "a" else "VL"
    ...
    try:
        ...
        if duration:
            ...
            if loop_duration <= trim_duration:
                print(f"Warning: Duration {loop_duration} <= trim duration {trim_duration}. No looping")
                os.rename(f"{temp_path}{extension}", output_path)
                print(f"Saved {'audio' if output_type == 'a' else 'video'} as {output_path.replace(os.sep, '/')}")
            else:
                loop_count = int(loop_duration // trim_duration) + (1 if loop_duration % trim_duration > 0 else 0)
                final_duration = min(loop_duration, loop_count * trim_duration)
                ffmpeg_command = (
                    f'ffmpeg -y -stream_loop {loop_count-1} -i "{temp_path}{extension}" '
                    f'-c:{"v copy" if output_type == "v" else "a copy"} -t {final_duration} "{output_path}"'
                )
                success, output = run_command(ffmpeg_command)
                if success:
                    print(f"Saved {'audio' if output_type == 'a' else 'video'} as {output_path.replace(os.sep, '/')}")
                else:
                    print(f"Loop failed for {actual_input}: {output}")
                    sys.exit(1)
                if os.path.exists(f"{temp_path}{extension}"):
                    os.remove(f"{temp_path}{extension}")
        else:
            os.rename(f"{temp_path}{extension}", output_path)
            print(f"Saved {'audio' if output_type == 'a' else 'video'} as {output_path.replace(os.sep, '/')}")
    finally:
